...
pub async fn dispatch(
    ...
    sqlx::query(
        "INSERT INTO webhook_deliveries (id, invoice_id, url, payload, status, attempts, last_attempt_at)
         VALUES (?, ?, ?, ?, 'pending', 1, ?)"
    )
    .bind(&delivery_id)
    .bind(invoice_id)
        ...
        .await
    {
        ...
        Ok(resp) => {
            tracing::warn!(invoice_id, event, status = %resp.status(), "Webhook rejected");
        }
        Err(e) => {
            tracing::warn!(invoice_id, event, error = %e, "Webhook failed");
        }
    }
...
pub async fn retry_failed(pool: &SqlitePool, http: &reqwest::Client) -> anyhow::Result<()> {
    let rows = sqlx::query_as::<_, (String, String, String, String)>(
        "SELECT wd.id, wd.url, wd.payload, m.webhook_secret
    ...
    for (id, url, payload, secret) in rows {
        ...
        match http.post(&url)
            .header("X-CipherPay-Signature", &signature)
            .header("X-CipherPay-Timestamp", &now)
            .json(&body)
            .timeout(std::time::Duration::from_secs(10))
            .send()
            .await
