...
pub async fn create_invoice(
    ...
    let price_eur = if currency == "EUR" {
        amount
    } else {
        price_zec * rates.zec_eur
    };
    let price_usd = if currency == "USD" {
        amount
    } else {
        price_zec * rates.zec_usd
    };
    ...
    let zcash_uri = if let Some(fc) = fee_config {
        ...
        if fee_amount >= MIN_FEE_ZEC {
            ...
            let fee_memo_b64 =
                base64::engine::general_purpose::URL_SAFE_NO_PAD.encode(fee_memo.as_bytes());
            format!(
                "zcash:?address={}&amount={:.8}&memo={}&address.1={}&amount.1={:.8}&memo.1={}",
                payment_address, price_zec, memo_b64, fc.fee_address, fee_amount, fee_memo_b64
            )
        } else {
            format!(
                "zcash:{}?amount={:.8}&memo={}",
    ...
    let price_zatoshis = zec_to_zatoshis(price_zec)?;
...
pub async fn finalize_invoice(
    ...
    if invoice.status != "draft" && invoice.status != "expired" {
        anyhow::bail!(
            "Invoice must be in draft or expired status to finalize (current: {})",
            invoice.status
        );
    }
